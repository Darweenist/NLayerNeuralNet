 ...
 * @author Dawson Chen
 * @since 2.18.2020
 * The NetworkBackpropABC class implements a neural network with 1 hidden layer, which allows the user to determine the number of activations
 ...
 */
public class NetworkBackpropABC
{
   /*
    ...
    */
   public static int setup(NetworkBackpropABC network, String[] args) throws IOException
   {
      /*
   ...
   public void evaluate()
   {
      for (int i = 0; i < numOutputs; i++)
      {
         ...
         for (int j = 0; j < numHidden; j++)
         {
            ...
            activations[1][j] = thresholdFunction(thetaj[j]);
            thetai[i] += activations[1][j] * weights[1][j][i];
         }
         activations[2][i] = thresholdFunction(thetai[i]);
         omegai[i] = Ti[i] - activations[2][i];
      ...
      for (int k = 0; k < numInputs; k++)
      {
         for (int j = 0; j < numHidden; j++)
         {
            ...
            for (int i = 0; i < numOutputs; i++)
            {
               ...
               weights[1][j][i] += learningFactor * activations[1][j] * psii[i];
            }
            uppercasePsij[j] = omegaj[j] * thresholdDerivative(thetaj[j]);
            weights[0][k][j] += learningFactor * activations[0][k] * uppercasePsij[j];
         } // for (int j = 0; j < numHidden; j++)
      } // for (int k = 0; k < numInputs; k++)
   } // public void train()
   ...
   {
      NetworkBackpropABC network = new NetworkBackpropABC(); // Initializes a Network object, on which following methods are run
