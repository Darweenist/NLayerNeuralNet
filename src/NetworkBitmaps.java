 ...
 * @author Dawson Chen
 * @since 2.18.2020
 * The NetworkBitmaps class implements a neural network with 2 hidden layers, which allows the user to determine the number of activations
...
public class NetworkBitmaps
{
   ...
   public int[] getActivationDimensions()
   {
      return activationDimensions;
   }
   ...
   public void setActivations(BufferedReader configReader, BufferedReader inputsReader) throws IOException
   {
      ...
      for (int inputInd = 0; inputInd < activationDimensions[0]; inputInd++)
      {
         ...
         activations[0][inputInd] = Double.parseDouble(inputsLine1);
      }
   } // public void setActivations(BufferedReader configReader, BufferedReader inputsReader) throws IOException
    ...
    */
   public static int setup(NetworkBitmaps network, String[] args) throws IOException
   {
      /*
   ...
   public static void main(String[] args) throws IOException
   {
      NetworkBitmaps network = new NetworkBitmaps(); // Initializes a Network object, on which following methods are run
      ...
      /*
       * This loop  prints out the truth table.
       */
...
//            totalError /= (double)(numTrainingSets);    // Makes sure that the total error is independent of the number of training sets used
         ...
         String outputFile = "finalWeights2";
         System.out.println("Weights writing into file..." + outputFile);
         ...
         for (int n = 0; n < network.numLayers - 1; n++)
         {
            for (int inputInd = 0; inputInd < network.activationDimensions[n]; inputInd++)
            {
               ...
               {
                  writer.write("w" + n + inputInd + outputInd + "=" + network.weights[n][inputInd][outputInd] + "\n");
               }
            }
         }
         writer.flush();
         System.out.println("All written into file and writer is flushed.");
      }

      // If the user just wants the network to evaluate the output values for this image, the code skips to here from the scanner program.
      for (int trainingSetLoopIndex = 0; trainingSetLoopIndex < numTrainingSets; trainingSetLoopIndex++)
      {
         /*
...
//         }
      ...
      else if (decision.equals("test") && numTrainingSets == 1)
      {
         System.out.print("Finished testing! Prediction: ");
         int prediction = 0;
         for (int i = 0; i < network.activationDimensions[network.numLayers - 1]; i++)
         {
            if (network.activations[network.numLayers - 1][i] > 0.5)
               prediction = i + 1;
         }
         if (prediction == 0)
            System.out.print("Not a number image.\n");
         else
