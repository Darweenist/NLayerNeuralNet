 ...
 * @author Dawson Chen
 * @since 2.18.2020
 * The NetworkBackprop3L class implements a neural network with 2 hidden layers, which allows the user to determine the number of activations
 * in the input, hidden, and output layers. It performs back propagation given hyperparameters that are specified by the user,
 ...
 * public static int setup(NetworkBackprop3L network, String[] args) throws IOException
 * public void evaluate()
 * public void train()
 * public static void main(String[] args) throws IOException
 */
public class NetworkBackprop3L
{
   /*
   ...
   public void setTruthValues(double[] truthValues)
   {
      for (int i = 0; i < activationDimensions[3]; i++)
      {
   ...
   public void setTrainingVariables()
   {
      omegai = new double[activationDimensions[3]];

      omegaj = new double[activationDimensions[2]];

      omegak = new double[activationDimensions[1]];

      thetai = new double[activationDimensions[3]];

      thetaj = new double[activationDimensions[2]];

      thetak = new double[activationDimensions[1]];

      psii = new double[activationDimensions[3]];

      psij = new double[activationDimensions[2]];

      psik = new double[activationDimensions[1]];
   } // public void setTrainingVariables()
   ...
   public void setWeights(BufferedReader weightsReader) throws IOException
   {
      ...
      /*
       * This loop fills the first layer of the weights 3-d array by looping through the input and first hidden layer activations.
      ...
      for (int m = 0; m < activationDimensions[0]; m++)
      {
         for (int k = 0; k < activationDimensions[1]; k++)
         {
            ...
            weights[0][m][k] = Double.parseDouble(line.substring(line.indexOf("=") + 1));
         }
      }

      /*
       * This loop fills the second layer of the weights 3-d array by looping through the hidden activations.
       * The outer loop loops through the first hidden layer, or left-hand side activations.
       * The inner loop loops through the second layer, or right-hand side activations.
       */
      for (int k = 0; k < activationDimensions[1]; k++)
      {
         for (int j = 0; j < activationDimensions[2]; j++)
         {
            String line = weightsReader.readLine();
            weights[1][k][j] = Double.parseDouble(line.substring(line.indexOf("=") + 1));
         }
      }

      /*
       * This loop fills the third layer of the weights 3-d array by looping through the hidden and output activations.
      ...
      for (int j = 0; j < activationDimensions[2]; j++)
      {
         ...
         {
            String line = weightsReader.readLine();
            weights[2][j][i] = Double.parseDouble(line.substring(line.indexOf("=") + 1));
         }
      }
   } // public void setWeights(BufferedReader weightsReader) throws IOException
   ...
   public void evaluate()
   {
      for (int k = 0; k < activationDimensions[1]; k++)
      {
         ...
         for (int m = 0; m < activationDimensions[0]; m++)
         {
            thetak[k] += activations[0][m] * weights[0][m][k];
         }
         activations[1][k] = thresholdFunction(thetak[k]);
      }
      ...
      for (int j = 0; j < activationDimensions[2]; j++)
      {
         ...
         for (int k = 0; k < activationDimensions[1]; k++)
         {
            thetaj[j] += activations[1][k] * weights[1][k][j];
         }
         activations[2][j] = thresholdFunction(thetaj[j]);
      }
      ...
      for (int i = 0; i < activationDimensions[3]; i++)
      {
         ...
         for (int j = 0; j < activationDimensions[2]; j++)
         {
            thetai[i] += activations[2][j] * weights[2][j][i];
         }
         activations[3][i] = thresholdFunction(thetai[i]);
         omegai[i] = Ti[i] - activations[3][i];
         Ei[i] = 0.5 * omegai[i] * omegai[i];
         psii[i] = omegai[i] * thresholdDerivative(thetai[i]);
      } // for (int i = 1; i < activationDimensions[3]; i++)
   } // public void evaluate()
   ...
   public void train()
   {
      for (int j = 0; j < activationDimensions[2]; j++)
      {
         ...
         for (int i = 0; i < activationDimensions[3]; i++)
         {
            ...
            weights[2][j][i] += learningFactor * activations[2][j] * psii[i];
         }
      }
      ...
      for (int k = 0; k < activationDimensions[1]; k++)
      {
         ...
         for (int j = 0; j < activationDimensions[2]; j++)
         {
            psij[j] = omegaj[j] * thresholdDerivative(thetaj[j]);
            omegak[k] += psij[j] * weights[1][k][j];
            weights[1][k][j] += learningFactor * activations[1][k] * psij[j];
         }
      } // for (int k = 0; k < activationDimensions[1]; k++)
      ...
      for (int m = 0; m < activationDimensions[0]; m++)
      {
         for (int k = 0; k < activationDimensions[1]; k++)
         {
            psik[k] = omegak[k] * thresholdDerivative(thetak[k]);
            weights[0][m][k] += learningFactor * activations[0][m] * psik[k];
         } // for (int k = 0; k < activationDimensions[1]; k++)
      } // for (int m = 0; m < activationDimensions[0]; m++)
   } // public void train()
   ...
   public static void main(String[] args) throws IOException
   {
      NetworkBackprop3L network = new NetworkBackprop3L(); // Initializes a Network object, on which following methods are run
       ...
       */
      double[][] truthValues = new double[numTrainingSets][network.activationDimensions[3]];

      for (int i = 0; i < network.activationDimensions[3]; i++)
      {
       ...
       */
      double[][] errors = new double[numTrainingSets][network.activationDimensions[3]]; // Stores the errors of the set of weights when run on each training set
      ...
      while (totalError > network.errorThreshold && iterationCounter < network.maxIterations)
      {
         ...
         totalError /= (double)(network.activationDimensions[3]); // Makes sure that the total error is independent of the number of output activations
